...
class MyTelnet(object):
    ...
    def __init__(self, timeout='3 seconds', newline='CRLF',
                 ...
                 terminal_type=None,monitor=False):
    ...
    def _get_connection(self, *args):
        ...
        return MyTelnetConnection(*args)
...
class MyTelnetConnection(telnetlib.Telnet):
    ...
    def __init__(self, host=None, port=23, timeout=3.0, newline='CRLF',
                 ...
                 terminal_emulation=False, terminal_type=None,monitor=False):
    ...
    def close_connection(self, loglevel=None):
        ...
        if self._monitor:
            self._cp_rb += self._decode(output)
            self._log(self._cp_rb,loglevel)
            output = self._cp_rb
            self._cp_rb = ''
        else:
        ...
        while time.time() < max_time:
            ...
            if out:
                if self._monitor:
                    self._cp_rb += out
                return True, out
        out = self._terminal_emulator.read()
        if self._monitor:
            self._cp_rb += out
        return False, out
