...
#----------------------------------------------------------------------
#
# Copyright (c) 2004,2005,2006,2007 by Kevin B. Kenny.  All rights reserved.
# See the file "license.terms" for information on usage and redistribution
# of this file, and for a DISCLAIMER OF ALL WARRANTIES.
#
# RCS: @(#) $Id: clock.tcl,v 1.9 2007/04/20 21:19:27 kennykb Exp $
#
#----------------------------------------------------------------------
...
# definition files.
# TODO - how to set this properly in the build dir ?

namespace eval tcl::clock \
    [list variable LibDir [file normalize [file dirname [info script]]]]
...
proc ::tcl::clock::LoadTimeZoneFile { fileName } {
    ...
    if { [catch {
	set f [open [file join $DataDir $fileName] r]
...
# Hide the original 'clock' command

rename clock ::tcl::clock::OldClock
...
if { [package vsatisfies [package provide Tcl] 8.5] } {
    ...
    namespace eval ::tcl::clock {
       foreach {subCommandMatcher subCommandDict} \
           [UniquePrefixRegexp {
               add          ::tcl::clock::add
               clicks       ::tcl::clock::clicks
               format       ::tcl::clock::format
               microseconds ::tcl::clock::microseconds
               milliseconds ::tcl::clock::milliseconds
               scan         ::tcl::clock::scan
               seconds      ::tcl::clock::seconds
           }] break
       set subCommandMatcher ^$subCommandMatcher\$
    }
    proc ::clock { sub args } {
       if { ![regexp $::tcl::clock::subCommandMatcher $sub] } {
           return -code error \
               "unknown option \"$sub\", must be add, clicks, format,\
                 microseconds, milliseconds, scan or seconds."
       }
       set cmd [dict get $::tcl::clock::subCommandDict $sub]
       uplevel [linsert $args 0 $cmd]
    }
}
