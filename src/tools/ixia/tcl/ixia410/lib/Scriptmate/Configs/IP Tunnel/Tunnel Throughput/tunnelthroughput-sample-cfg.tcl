# Tunnel/tunnelThroughput
##################################################################################
# Version 3.70	$ Revision:  $
# $Date: 12/12/02 2:24p $
# $Author: Dheins $
#
# $Workfile: testTunnelTP.tcl $ - sample user script for running the Tunnel Throughput test
#
#   Copyright © 1997 - 2004 by IXIA
#   All Rights Reserved.
#
# Description:
#  This test determines the maximum rate at which the DUT (Device under Test)
#  receives and forwards frames without any loss of frames while tunneling traffic. 
#  Frames are initially sent at a user-specified rate, generally the maximum 
#  theoretical rate based on the speed of the port. A binary search algorithm is 
#  executed to obtain a rate at which the DUT does not lose frames. This test is
#  configured with a one-to-one traffic mapping.
#
#  Results from the throughput test will be written to the file 'ipv6TunnelTput.results'.
#  This is an ascii text-based file that may be viewed via Notepad or your
#  favorite editor, or it may be printed out as a formal report.
#
##################################################################################
...
logOn ipv6TunnelThruput.log

logMsg "\n\n  IPv6 Tunnel Throughput test"
