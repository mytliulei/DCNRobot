	...
	namespace eval ::safe {
		...
		switch [llength $args] {
			...
			if {(![::tcl::OptProcArgGiven -statics])  && (![::tcl::OptProcArgGiven -noStatics]) } {
			    set statics    [Set [StaticsOkName $slave]]
			} else {
			    set statics    [InterpStatics]
			}
			if {([::tcl::OptProcArgGiven -nested])  || ([::tcl::OptProcArgGiven -nestedLoadOk]) } {
		...
		::interp alias $slave glob {} [namespace current]::AliasGlob  $slave
		...
		if {[catch {::interp eval $slave {source [file join $tcl_library init.tcl]}} msg]} {
		    Log $slave "can't source init.tcl ($msg)"
		    error "can't source init.tcl into slave $slave ($msg)"
		}

		if {[catch {::interp eval $slave  {source [file join $tcl_library tm.tcl]}} msg]} {
		...
		foreach dir $pathList {
			...
			foreach sub [glob -directory $dir -nocomplain *] {
			    if {([file isdirectory $sub])  && ([lsearch -exact $res $sub]<0) } {
				# new sub dir, add it !
				lappend res $sub
			    }
			}
		...
		if {[catch {::interp invokehidden $slave load $file $package $target} msg]} {
		    Log $slave $msg
		    return -code error $msg
		}
		...
		if {[regexp $okpat $subcommand]} {
		    return [eval [linsert $args 0  ::interp invokehidden $slave encoding]]
		}

		if {[string first $subcommand system] == 0} {
		    if {$argc == 1} {
			# passed all the tests , lets source it:
			if {[catch {::interp invokehidden  $slave encoding system} msg]} {
			    Log $slave $msg
			    return -code error "script error"
			}
		    } else {
